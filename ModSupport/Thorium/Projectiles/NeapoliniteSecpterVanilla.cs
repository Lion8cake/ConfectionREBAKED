...
namespace TheConfectionRebirth.ModSupport.Thorium.Projectiles
{
	...
	public class NeapoliniteScepterVanilla : ThoriumMod.Projectiles.ThoriumProjectile {
		...
		public override void AI() {
			...
			for (int num363 = 0; num363 < 3; num363++) {
				...
				int num369 = Dust.NewDust(Projectile.position, Projectile.width, Projectile.height, ModContent.DustType<Dusts.NeapoliniteVanillaDust>(), 0f, 0f, 150, default(Color), 1.75f);
		...
		public override bool OnTileCollide(Vector2 oldVelocity) {
			for (int j = 0; j < 10; j++) {
				int dust = Dust.NewDust(Projectile.position, Projectile.width, Projectile.height, ModContent.DustType<Dusts.NeapoliniteVanillaDust>(), (float)Main.rand.Next(-2, 2), (float)Main.rand.Next(-2, 2), 150, default(Color), 1.5f);
