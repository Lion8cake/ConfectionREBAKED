...
namespace TheConfectionRebirth.ModSupport.Thorium.Projectiles
{
	...
	public class NeapoliniteMegaphonePro : ThoriumMod.Projectiles.Bard.BardProjectile {
		...
		public override void AI() {
			...
			if (Projectile.ai[1] >= 0f) {
				...
				for (int i = 0; i < numDusts; i++) {
					...
					Dust obj = Dust.NewDustPerfect(Projectile.Center + offset, 86, (Vector2?)Utils.SafeNormalize(offset, Vector2.UnitY), 0, default(Color), 1f);
					obj.scale = 0.75f;
					obj.noGravity = true;
				}
				Projectile.ai[1] = -3f;
