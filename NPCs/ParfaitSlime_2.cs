...
namespace TheConfectionRebirth.NPCs
{
    ...
    public class ParfaitSlime_2 : ModNPC
    {
        ...
        {
            if (spawnInfo.Player.ZoneRockLayerHeight && spawnInfo.Player.ZoneSnow && spawnInfo.Player.InModBiome(ModContent.GetInstance<ConfectionBiomeSurface>()) && !spawnInfo.Player.ZoneOldOneArmy && !spawnInfo.Player.ZoneTowerNebula && !spawnInfo.Player.ZoneTowerSolar && !spawnInfo.Player.ZoneTowerStardust && !spawnInfo.Player.ZoneTowerVortex && !spawnInfo.Invasion)
            {
                return 1.0f;
            }
            return 0f;
