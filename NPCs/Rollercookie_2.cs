...
namespace TheConfectionRebirth.NPCs
{
    public class Rollercookie_2 : ModNPC
    {
        ...
        {
            if (spawnInfo.Player.ZoneOverworldHeight && spawnInfo.Player.ZoneSnow && spawnInfo.Player.InModBiome(ModContent.GetInstance<ConfectionBiomeSurface>()) && !spawnInfo.Player.ZoneOldOneArmy && !spawnInfo.Player.ZoneTowerNebula && !spawnInfo.Player.ZoneTowerSolar && !spawnInfo.Player.ZoneTowerStardust && !spawnInfo.Player.ZoneTowerVortex && !spawnInfo.Invasion)
            {
                return 0.1f;
            }
            return 0f;
